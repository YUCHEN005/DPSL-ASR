...
for x in utt2spk spk2utt feats.scp text segments wav.scp spk1.scp cmvn.scp vad.scp \
...
function filter_recordings {
  ...
  # recording-ids.
    ...
    filter_file $tmpdir/recordings $data/spk1.scp
...
function filter_utts {
  ...
  [ ! -f $data/segments ] && maybe_wav1=spk1.scp # wav indexed by utts only if segments does not exist.
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames $maybe_wav $maybe_wav1 $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
