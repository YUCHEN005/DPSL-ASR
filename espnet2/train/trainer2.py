...
###            if module._get_name() == 'RNNSeparator':
...
###                    logging.info(f"param should be no grad , {param}")
    ...
    def train_one_epoch(
        ...
        #    logging.info(f"model.children is layer is {layer} ")
        #for iiter, (_, batch) in enumerate(
        for iiter, (uttid, batch) in enumerate(
            reporter.measure_iter_time(iterator, "iter_time"), 1
        ):
            assert isinstance(batch, dict), type(batch)
            logging.info(f"in the train_one_epoch , uttid is {uttid} and batch is {batch}")
            if distributed:
