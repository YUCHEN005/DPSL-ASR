...
class Trainer:
    ...
    def run(
        ...
        # Check(md): remove specify layer gradient graph from model.
        #for name, param in dp_model.named_parameters():
        #    logging.info(f"in the trainer1.py,without remove enh_model gradient from the joint_enh_and asr,{name} is {param}")
        #    logging.info(f"in the trainer1.py,without remove enh_model gradient from the joint_enh_and asr,name is {name}")
        #    if name in ["module.enh_model"]:
        #        param.required_grad=False
        #        logging.info(f"in the trainer1.py, enh_model remove gradient from the joint_enh_and asr, {param}")

        # md 2021-3-2
        for module in dp_model.modules():
            #logging.info(f"dp_model contain module is {module}")
            if module._get_name() == 'TFMaskingNet1':
