...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, max_epoch + 1):
            ...
            for iiter, (_,batch) in enumerate(train_iter_factory.build_iter(iepoch)):
                # 1. Train  for one-iter
                logging.info(f"{iiter}iter/{iepoch}epoch !!!!!!!!!")
                with reporter.observe("train") as sub_reporter:
                    all_steps_are_invalid = cls.train_one_iter(
                        model=dp_model,
                ...
                if not distributed_option.distributed or distributed_option.dist_rank == 0:
                    ...
                    torch.save(model.state_dict(), output_dir / f"{iiter}iter.pth")
                    # Creates a sym link latest.pth -> {iepoch}epoch.pth
                    p = output_dir / "latest.pth"
                    if p.is_symlink() or p.exists():
                        p.unlink()
                    p.symlink_to(f"{iiter}iter.pth")
                    #_improved = []
                    #for _phase, k, _mode in best_model_criterion:
                    #    # e.g. _phase, k, _mode = "train", "loss", "min"
                    #    if reporter.has(_phase, k):
                    #        best_iter = reporter.get_best_epoch(_phase, k, _mode)
                    #        logging.info(f"best_iter is {best_iter}")
                    ...
                    #if len(_improved) == 0:
                    #    logging.info("There are no improvements in this iter")
                    #else:
                    #    logging.info(
                    ...
                    #    ]
                    #)
                    #for e in range(1, iiter):
                    #    p = output_dir / f"{e}iter.pth"
                    ...
                    logging.info(f"all_steps_are_invalid is {all_steps_are_invalid}")
